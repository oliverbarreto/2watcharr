...
#### [MODIFY] [migrations.ts](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/db/migrations.ts)
- The `add_is_short` migration will also include a data update step:
  - Run an SQL update to mark episodes with `/shorts/` in their URL as `is_short = 1`.
  - This ensures that when the Docker container starts and runs migrations, all existing data is automatically updated without manual intervention.
