...
- Adjust the "Confirm Watched" button styling:
    - Reduce font size when `watchStatus === 'pending'`.
    - Use `text-[10px]` or `text-xs` for the "Confirm Watched" label to save space.
    - Ensure the button container and ellipsis trigger have adequate spacing and don't overflow.
...
- **Card View**:
    ...
    7. Find an episode in "Pending Confirmation" state.
    8. Verify the "Confirm Watched" button text is smaller and the ellipsis button is clearly visible.
