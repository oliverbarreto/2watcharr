...
Update [setPriority](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/services/media.service.ts#318-328) and [updateEpisode](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/services/media.service.ts#161-185) to automatically call [moveToBeginning](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/services/media.service.ts#332-338) when an episode is marked as `high` priority. This ensures that prioritizing an episode via any API endpoint (including the context menu) correctly places it at the top of the Watch Next queue.

Also, update [saveEpisodeFromMetadata](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/services/media.service.ts#42-140) to call [moveToBeginning](file:///Users/oliver/_HD_LOCAL/dev/2watcharr/src/lib/services/media.service.ts#332-338) for **newly created** episodes. This ensures that if a new episode is immediately given priority (or if the user expects new items at the top of a custom-sorted list), it is correctly positioned.

```typescript
// Proposed change for updateEpisode
...
// Proposed change for saveEpisodeFromMetadata (at the end)
// ... creation logic ...
