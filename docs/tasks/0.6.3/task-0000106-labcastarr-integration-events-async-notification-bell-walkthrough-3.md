...
- **Notification Bell V2 Improvements**:
  - **Immediate Feedback**: The bell now listens for the `episode-added` event, triggering an instant refresh of the unread count when a new episode is requested.
  - **Faster Polling**: Polling interval reduced from 15s to 5s for more responsive background updates.
  - **Automatic Badge Clearing**: Clicking the bell now automatically marks all notifications as read and clears the badge immediately before navigating to the Activity page.
- Achieved a perfectly clean `npm run lint` and `npm run typecheck` state.
