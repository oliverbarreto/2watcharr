# TASK: Update Tag Release to 0.6.5

We want to bump to version 0.6.5. This is the full plan:
...
// turbo 3. **Version Bump**: Bump the version in `package.json` using the provided argument.

- `npm version {{1}}`

// turbo 4. **Commit**: Commit `develop` branch before moving to `main`.

- `git add --all && git commit -m "Run tests, fixed linting errors and updated documentation before merge/tag/release into main"`

// turbo 5. **Merge**: Merge the `develop` branch into `main`.

- `git checkout main`
- `git merge develop`

// turbo 6. **Release**: Push to GitHub and create a release using the GitHub CLI (`gh`).
