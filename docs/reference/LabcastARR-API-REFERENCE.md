...
4. [API Endpoints](#api-endpoints)
   ...
   - [Integration API](#integration-api-endpoints)
...
The Integration API allows external applications to create podcast episodes programmatically using a user-generated token. See the [Integration API Guide](./INTEGRATION-API-GUIDE.md) for the full developer guide including setup instructions and code examples.

### Authentication

Integration endpoints use a separate header — **not** a JWT Bearer token:

```http
X-Integration-Token: YOUR_TOKEN_HERE
```

Tokens are generated per-user from **Settings → Integration** in the LabCastARR UI. One active token per user. The `X-API-Key` header is **not** required for these endpoints.

### Token Management Endpoints

> These require a valid JWT session (logged-in user). They are for token lifecycle management, not for external apps.

| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| `GET` | `/v1/integration/token` | JWT | Get current token status (masked display) |
| `POST` | `/v1/integration/token` | JWT | Generate new integration token (raw token returned once) |
| `DELETE` | `/v1/integration/token` | JWT | Revoke and delete the current token |
| `POST` | `/v1/integration/token/regenerate` | JWT | Revoke old token and issue a new one |

**GET `/v1/integration/token`** — Response:
```json
{
  "is_active": true,
  "masked_token": "abcd****...****wxyz",
  "created_at": "2026-03-04T10:00:00Z",
  "last_used_at": "2026-03-04T12:30:00Z"
}
```

**POST `/v1/integration/token`** — Response 201:
```json
{
  "token": "full-raw-token-shown-once-only",
  "masked_token": "abcd****...****wxyz",
  "created_at": "2026-03-04T10:00:00Z"
}
```

### Episode Creation Endpoints

> These use `X-Integration-Token` authentication (for external apps).

| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| `POST` | `/v1/integration/episodes` | Integration Token | Create a single episode from a YouTube URL |
| `POST` | `/v1/integration/episodes/bulk` | Integration Token | Create 1–50 episodes from a list of YouTube URLs |

**POST `/v1/integration/episodes`** — Request body:
```json
{
  "channel_id": 1,
...
```
Response 201: Full episode object with `"status": "pending"`.

**POST `/v1/integration/episodes/bulk`** — Request body:
```json
{
  "channel_id": 1,
  "video_urls": ["https://youtube.com/watch?v=ID1", "https://youtube.com/watch?v=ID2"],
  "tags": [],
...
{
  "results": [
    { "video_url": "...", "success": true, "episode_id": 42, "error": null },
    { "video_url": "...", "success": false, "episode_id": null, "error": "..." }
  ],
  "created": 1,
  "failed": 1
}
```
The bulk endpoint always returns 200 OK. Check each `result.success` field for per-URL outcomes.
