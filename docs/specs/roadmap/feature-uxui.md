...
- [ ] UI/UX: some ui modifications we need to implement:
    - [ ] PLAN MODE: We now want to extend the model to add, if not already being saved, important dates of videos (eg: when added to the list, when watched, when favorited, when removed, etc.). 
